...
from diffusion_policy.real_world.xarm6_interpolation_controller import XARM6InterpolationController
...
DEFAULT_OBS_KEY_MAP = {
    # robot --> there is no targetTCPPose
...
class RealEnv:
    def __init__(self, 
            ...
            max_rot_speed=0.6,
            # robot
            # tcp_offset=0.13, #check this for xarm
            tcp_offset = 0.195,
            init_joints=False,
        ...
        #     )
        """for XARM6"""
        robot = XARM6InterpolationController(
            shm_manager=shm_manager,
            robot_ip=robot_ip,
            frequency=100, # CHANGE THIS FOR XARM6
            lookahead_time=0.1,
            gain=300,
            max_pos_speed=max_pos_speed*cube_diag,
            max_rot_speed=max_rot_speed*cube_diag,
            launch_timeout=3,
            tcp_offset_pose=[0,0,tcp_offset,0,0,0], #ONLY OFFSET IN z AXIS
            payload_mass=None,
            payload_cog=None,
            joints_init=j_init,
            joints_init_speed=1.05,
    ...
    def start(self, wait=True):
        ...
        self.robot.start(wait=False) #double check
    ...
    def stop(self, wait=True):
        ...
        self.robot.stop(wait=False) #double check
    ...
    def get_obs(self) -> dict:
        ...
        """important double check for xarm6"""
        ...
        for k, v in last_robot_data.items():
            if k in self.obs_key_map:
                #only the first 4 keys are used: ActualTCPPose,ActualTCPSpeed, ActualQ,ActualQd 
    ...
    """important check!!! action is [x,y,z]!!!!"""
    def exec_actions(self, 
        ...
        # print(f'self.action_accumulator:{self.action_accumulator is not None}')
        # print(f'self.stage_accumulator:{self.stage_accumulator is not None}')
        # record actions
    ...
    def get_robot_state(self):
        return self.robot.get_state() #double check
